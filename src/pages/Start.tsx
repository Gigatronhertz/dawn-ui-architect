...
function ConfirmStep({ botNumber, destination, dmSent, tripId }: {
  ...
  const waLink = `https://wa.me/${botNumber.replace(/\D/g, "")}`;
    ...
    <Card>
      <div className="text-center mb-8">
        ...
        <p className="text-muted-foreground mt-2">
          {dmSent
            ? "Check your WhatsApp — we just sent you the add-to-group instructions."
            : `Now add the bot to your ${destination} squad's WhatsApp group.`}
        </p>
      </div>

      {/* Primary CTA when DM was sent */}
      {dmSent && (
        <a
          href={waLink}
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center justify-center gap-2 rounded-2xl bg-whatsapp text-white px-5 py-4 text-sm font-semibold mb-6 hover:scale-[1.01] active:scale-[0.99] transition-transform"
        >
      ...
      {/* Step-by-step for users without the DM */}
      {!dmSent && (
